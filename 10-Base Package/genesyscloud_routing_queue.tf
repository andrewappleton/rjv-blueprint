resource "genesyscloud_routing_queue" "RJV_Menhir_Email" {
  ...
  members {
    ring_num = 1    
    user_id = data.genesyscloud_user.ref_userid.id # Perhaps use a group rather than a specific user?  
  }
  ...
  name = "${var.RESOURCE_PREFIX}_Menhir_Email"
...
resource "genesyscloud_routing_queue" "RJV_Menhir_Voice" {
  ...
  name = "${var.RESOURCE_PREFIX}_Menhir_Voice"
  outbound_email_address {
    domain_id = data.genesyscloud_routing_email_domain.ref_email_domain.id
    route_id  = "${genesyscloud_routing_email_route.testRJV_mypurecloud_ie.id}"
  }
